...
- البيانات: أسئلة وتصنيفات محفوظة في قاعدة SQLite. عند أول تشغيل يتم استيراد البذور من `server/data/questions.json` ثم تصبح لوحة الإدارة متصلة بقاعدة البيانات.
...
QUESTION_DB_PATH=server/data/questions.sqlite
MIN_PLAYERS=3
MAX_PLAYERS=6
ANSWER_SECONDS=30
VOTE_SECONDS=30
...
في الإنتاج اجعل `QUESTION_DB_PATH` يشير إلى ملف داخل مساحة تخزين دائمة، مثل `/data/questions.sqlite`، حتى لا تضيع الأسئلة التي تضيفها من لوحة الإدارة عند إعادة النشر.
...
حقول طور الأقلية تكسب:

```json
{
  "mode": "minority_wins",
  "category": "الأقلية تكسب",
  "prompt": "اختر مكانا، الخيار الأقل تصويتا يفوز.",
...
- إرسال الإجابة الصحيحة حرفيا أثناء مرحلة الإجابة لا يعطي نقاطا؛ يطلب منك كتابة خدعة بدلا منها.

الأطوار الحالية المدارة من لوحة الإدارة هي: فلتة، الدخيل، كذبة ذكية، الأقلية تكسب، والحكم.
